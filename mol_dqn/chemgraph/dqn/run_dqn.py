...
class MultiObjectiveRewardMolecule(molecules_mdp.Molecule):
  ...
  #def get_CNSMPO(self, latent_vector, session):
  #  
  #  feed_dict = {
  #    x_placeholder: latent_vector
  # }
  #  
  #  predCNSMPO = session.run(estimated_targets, feed_dict=feed_dict)
  #  
  #  return predCNSMPO
  #
  #def get_VINA(self, latent_vector, session):
  #  """does the vina thing"""
  #  feed_dict = {
  #    x_placeholder: latent_vector
  #  }
  #  
  #  predVINA = session.run(estimated_targets, feed_dict=feed_dict)
  #  
  #  return predVINA
  ...
  def _reward(self):
    ...
    GetVocab(self._state)
    molvector = GetLatentVector(self._state, vocab, 450, 56, 3, JTVAE)
    vina_score = get_VINA(molvector, vinaSession)
    cnsmpo_score = get_CNSMPO(molvector, cnsmpoSession)
    
    reward = ( vina_score * self._sim_weight + cnsmpo_score * (1 - self._sim_weight) )
...
def run_training(hparams, environment, dqn):
  ...
  """
  Add a way to load in the model previously trained,
  and a way to output smiles strings
  """
  with tf.Session() as sess:
    dqn.build()
    
    model_saver = tf.train.Saver(max_to_keep=hparams.max_num_checkpoints) # commented out to allow loading?
    ...
    if hparams.prioritized:
      memory = PrioritizedReplayBuffer(hparams.replay_buffer_size,
    ...
    # saver = tf.train.import_meta_graph('/home/jaredwg/Documents/google-research/mol_dqn/chemgraph/dqn/save/ckpt-60000.meta') # added to allow loading
    # saver.restore(sess,tf.train.latest_checkpoint('/home/jaredwg/Documents/google-research/mol_dqn/chemgraph/dqn/save/')) # added to allow loading
...
def _episode(environment, dqn, memory, episode, global_step, hparams,
  ...
  for step in range(hparams.max_steps_per_episode):
    ...
    if step == hparams.max_steps_per_episode - 1:
      ...
      logging.info('The reward is: %s', str(float(result.reward)))
      
    
    with open('SMILES.csv','a') as fd:
      fd.write(result.state + "," + str(float(result.reward)) + "\n")
